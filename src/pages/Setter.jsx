    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        {eodLoading ? <SkeletonCard /> : reports.length === 0 ? (
          ...
          <div className="space-y-2">
            {reports.map(r => r.role === 'setter' ? <SetterEODHistoryItem key={r.id} report={r} /> : <GenericEODItem key={r.id} report={r} />)}
          </div>
        )}
      </div>
    </div>
...
function EODSection({ userId }) {
  const [open, setOpen] = useState(false)
  return (
    <div className="mb-6">
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between px-4 py-3 bg-white border border-[#e5e7eb] rounded-xl hover:border-[#6366f1]/30 transition-colors"
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-[#1a1a1a]">Rapport End of Day</span>
        </div>
        <svg className={`w-4 h-4 text-[#6b7280] transition-transform ${open ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
        </svg>
      </button>
      {open && (
        <div className="mt-2">
          <SetterEODForm userId={userId} />
        </div>
      )}
    </div>
  )
