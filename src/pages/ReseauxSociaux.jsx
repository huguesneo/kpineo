...
function PerformanceView({ accounts, accountsError, posts, onPostClick }) {
  ...
  const published = useMemo(
    () => posts.filter(p => p.status === 'publie').sort((a, b) => (b.published_at ?? '').localeCompare(a.published_at ?? '')),
    [posts],
  )
    ...
    <div className="space-y-5">
      ...
      <Card className="p-5">
        <h3 className="text-base font-bold text-[#1a1a1a] mb-3">
          Contenus publiés <span className="text-sm font-semibold text-[#9ca3af]">({published.length})</span>
        </h3>
        <div className="divide-y divide-[#f0f0f2]">
          {published.map(p => (
            <button key={p.id} onClick={() => onPostClick(p)} className="w-full flex items-center gap-3 py-2.5 text-left hover:bg-[#f9fafb] px-2 rounded-lg transition-colors">
          ...
          {!published.length && <p className="text-sm text-[#9ca3af] py-2">Aucun contenu publié pour l'instant.</p>}
        </div>
      </Card>
    </div>
  )
